...
\section{Finding Happiness}
    ...
    \subsection{Problem Definition}
        ...
        Ideally an algorithm $A(n)$ would iteratively select the index with the highest likelihood of being a hit, with $n$ being the number of iterations. This algorithm would return a set of indicies $i_a \in A(n)$, where each index was a hit $\forall i_a, H(i_a) = 1$.
        \br
        The problem is defined as maximizing the score of an algorithm $S(A, n)$, which is defined as the proportion of hits per iteration\br
